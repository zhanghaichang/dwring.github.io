...
<span style="font-size:18px">synchronized：</span>
...
     五、以上规则对其它对象锁同样适用。
<span style="font-size:18px">区别：</span>
...
<div style="white-space:pre-wrap; text-align:left; line-height:1.75; font-size:14px"><span style="font-size:14px">三、</span>volatile 变量可以被看作是一种 “程度较轻的 synchronized”；与 synchronized 块相比，volatile 变量所需的编码较少，并且运行时开销也较少，但是它所能实现的功能也仅是 synchronized 的一部分。Volatile 变量具有 synchronized 的可见性特性，但是不具备原子特性（也就是不具有互斥性）。
...
<span style="font-size:18px">拓展：</span>
...
<span style="font-size:14px">主要是因为Java API强制要求这样做，如果你不这么做，你的代码会抛出IllegalMonitorStateException异常。还有一个原因是为了避免wait和notify之间产生竞态条件。
</span>
